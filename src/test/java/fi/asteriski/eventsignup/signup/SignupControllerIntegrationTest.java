...
class SignupControllerIntegrationTest {
    ...
    void getExistingEventForSignup() throws Exception {
        ...
        when(signupService.getEventForSignUp(anyString(), any(Locale.class))).thenReturn(event);
        mockMvc.perform(get("/signup/123")).andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
            .andExpect(content().json(jsonString, true));
        verify(signupService).getEventForSignUp(eq("123"), any(Locale.class));
    }

    @Test
    @DisplayName("Get non-existing event for signup.")
    void getNonExistingEventForSignup() throws Exception {
        when(signupService.getEventForSignUp(anyString(), any(Locale.class))).thenThrow(EventNotFoundException.class);
        mockMvc.perform(get("/signup/456")).andExpect(status().isNotFound());
        verify(signupService).getEventForSignUp(eq("456"), any(Locale.class));
    }

    @Test
    @DisplayName("Get existing event for signup where event is full.")
    void getExistingEventForSignupWhichIsFull() throws Exception {
        when(signupService.getEventForSignUp(anyString(), any(Locale.class))).thenThrow(EventFullException.class);
        mockMvc.perform(get("/signup/789")).andExpect(status().isConflict());
        verify(signupService).getEventForSignUp(eq("789"), any(Locale.class));
    }
    ...
    void getExistingEventForSignupWhereSignupHasNotStarted() throws Exception {
        when(signupService.getEventForSignUp(anyString(), any(Locale.class))).thenThrow(SignupNotStartedException.class);
        mockMvc.perform(get("/signup/1234")).andExpect(status().isConflict());
        verify(signupService).getEventForSignUp(eq("1234"), any(Locale.class));
    }
    ...
    void getExistingEventForSignupWhereSignupHasEnded() throws Exception {
        when(signupService.getEventForSignUp(anyString(), any(Locale.class))).thenThrow(SignupEndedException.class);
        mockMvc.perform(get("/signup/456123")).andExpect(status().isConflict());
        verify(signupService).getEventForSignUp(eq("456123"), any(Locale.class));
    }
